...
func newViewStatusCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			nodeResult, err := runtime.server.ExecuteTool(ctx, "get_node_status",
				map[string]any{"include_metrics": true})
			if err != nil {
				return fmt.Errorf("get_node_status: %w", err)
			}
			podResult, err := runtime.server.ExecuteTool(ctx, "get_pod_resources",
				map[string]any{"problematic_only": false, "include_containers": false})
			if err != nil {
