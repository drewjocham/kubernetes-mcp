...
## 2. AI Code Review Template (MANDATORY)
When performing a review, AI agents must evaluate and report using these criteria:
...
### B. Coding Style & Golang Best Practices
...
* **Decision Tracing:** New features must be added to the `ID,Decision,Impact,Proposal,Date,Status` table in `DECISION_LOG.md`.
* **Test Commands:** * `make test`: Run all unit tests.
    * `make lint`: Execute `golangci-lint` (must pass 100%).
    * `make test-integration-channel`: Local integration test for the streaming log feature.
    * `make run-watcher`: Run watcher engine locally for testing.
    * `make run-server`: Run MCP HTTP server locally.

---

### Implementation Note
To activate this, place this file in your root directory. If you are using **Cursor**, you can also add a reference to this file in your `.cursorrules` to ensure the "AI Agent Code Review Template" is automatically triggered during every "Chat" or "Edit" session.
