// Test with fallback regex only (simulating SSR)
...
function htmlToMarkdown(input) {
  ...
  console.log('After decode:', JSON.stringify(text))
  
  // Step 1: Handle <br> tags - convert to newlines
  text = text.replace(/<br\s*\/?>\s*<br\s*\/?>/gi, '\n\n')
  text = text.replace(/<br\s*\/?>/gi, '\n')
  console.log('After br handling:', JSON.stringify(text))
  ...
  console.log('After pattern fix:', JSON.stringify(text))
  ...
  console.log('After tag conversion:', JSON.stringify(text))
  
  // Step 4: Remove any remaining HTML tags
  text = text.replace(/<[^>]*>/g, '')
  console.log('After tag removal:', JSON.stringify(text))
  
  // Step 5: Clean up markdown formatting issues
  text = text.replace(/```(\w+)\s*\n\s*```/g, '```$1\n```')
  text = text.replace(/\n{3,}/g, '\n\n')
  console.log('After cleanup:', JSON.stringify(text))
